...
- S5 — Live probes this session (Redshift Data API, Databricks Statement Execution API),
  2026-08-28.
...
| Field | Value | Status |
|---|---|---|
| Source engine | Amazon Redshift Serverless, version 1.0.417950 (`SELECT version()`), workgroup `demo-wg`, db `demo`, us-east-1 | FACT (S5) |
...
| Branch/PR model | `main` is the pristine pre-migration estate — never merge migration PRs into `main`. Each run uses a `migration-run-N` integration branch; unit branches PR into it. This engagement: `migration-run-6` | FACT (S4) |
...
| Stored procedures | Redshift PL/pgSQL (`sp_refresh_marts`) is not portable; re-platform as an ordered job/workflow running mart builds 10 → 11 | FACT (S1: `databricks/etl/12_refresh_marts_job.sql`, S2) |
...
| Untracked consumers | Query-history sweep is the detection source; `sys_query_history` is accessible but sparse (8 rows at probe — serverless retention + low demo activity), so consumer-detection evidence is WEAK. Registered as a D4-evidence gap | FACT (S5) |
