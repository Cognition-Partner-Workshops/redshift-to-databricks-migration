...
-- Redshift DECIMAL division truncates to the result scale (4); Databricks CAST rounds, so the
-- quotient is truncated explicitly with FLOOR(x, 4).
...
SELECT
    ...
    CAST(FLOOR(SUM(o.order_total) / NULLIF(COUNT(DISTINCT o.order_id), 0), 4) AS DECIMAL(38,4))
                                                        AS avg_order_value
