...
--   AVG over DECIMAL(12,2) -> Redshift keeps the input scale and truncates, while Databricks
--                             widens the scale and rounds. Reproduced as SUM/COUNT truncated
--                             toward zero at scale 2.
--   DATEDIFF(day, a, b)    -> Redshift counts day-boundary crossings: DATEDIFF(date(b), date(a))
--   DISTKEY/SORTKEY        -> dropped (Delta), and DROP + CREATE -> CREATE OR REPLACE
